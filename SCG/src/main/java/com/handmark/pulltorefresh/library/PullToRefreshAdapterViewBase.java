...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
	 ...
	 * when the Adapter View is scrolled to the top and the mode is set to {@link Mode#PULL_FROM_START}. The default value is <var>true</var> if {@link PullToRefreshBase#isPullToRefreshOverScrollEnabled()
	 ...
	 * getRefreshableView()}. {@link AdapterView#setAdapter(android.widget.Adapter)} setAdapter(adapter)}. This is just for convenience!
	 *
	 * @param adapter
	 *            - Adapter to set
	 */
	 ...
	 * a state where a Pull-to-Refresh can happen. An example of this state is
	 * when the Adapter View is scrolled to the top and the mode is set to {@link Mode#PULL_FROM_START}
	 *
	 * @param showIndicator
