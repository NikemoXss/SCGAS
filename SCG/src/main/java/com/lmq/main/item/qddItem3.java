...
public class qddItem3 {
	...
	public void initTransfer(JSONObject json) {
		...
		try {
			/*
			 * if (json.has("SecondaryJsonList")) SecondaryJsonList = new
			 * JSONArray(); if (json.has("LoanOutMoneymoremore"))
			 * LoanOutMoneymoremore = json.getString("LoanOutMoneymoremore");//
			 * 付款人钱多多标识 if (json.has("LoanInMoneymoremore")) LoanInMoneymoremore
			 * = json.getString("LoanInMoneymoremore");// 收款人钱多多标识 if
			 * (json.has("OrderNo")) OrderNo = json.getString("OrderNo");//
			 * 网贷平台订单号 if
			 * (json.has("BatchNo")) BatchNo =
			 * json.getString("BatchNo");// 网贷平台标号
			 * if (json.has("money")) Amount
			 * = json.getString("money");// 金额 if (json.has("Remark")) Remark =
			 * json.optString("Remark", "");// 备注 if (json.has("FullAmount"))
			 * FullAmount = json.optString("FullAmount", "");// 满标 if
			 * (json.has("TransferName")) TransferName =
			 * json.optString("TransferName", "");// 转账用途 if
			 * (json.has("SecondaryJsonList") &&
			 * !json.isNull("SecondaryJsonList"))// 二次分配列表 SecondaryJsonList =
			 * json.getJSONArray("SecondaryJsonList");
			 */
			...
			// PlatformMoneymoremore = LoanJsonList.getJSONObject(0).getString(
			// "LoanInMoneymoremore");
			PlatformMoneymoremore = json.getString("PlatformMoneymoremore");// 平台乾多多标识;
			TransferAction = json.optString("TransferAction", "1");// 转账类型 1.投标
																	// 2.还款 3.其他
			Action = json.optString("Action", "1");// 操作类型 1.手动转账 2.自动转账
			TransferType = json.optString("TransferType", "2");// 转账方式 1.桥连 //
																// 2.直连
