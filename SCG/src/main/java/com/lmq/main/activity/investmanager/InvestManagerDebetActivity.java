...
public class InvestManagerDebetActivity extends BaseActivity implements OnClickListener {
		...
		popList.setOnItemClickLinstener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				case 3:
					...
					break;
				case 4:
					requestURL = Default.ondetb;

					break;
				case 5:
					requestURL = Default.cancellist;
					break;
				}
	...
	class Adapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (null == convertView) {
				...
				convertView = View.inflate(InvestManagerDebetActivity.this, R.layout.invest_manager_debet_item, null);
				...
				viewHolder.invest_manager_item_dbjg2 = (TextView) convertView.findViewById(R.id.invest_manager_item_dbjg2);
			...
			viewHolder.invest_manager_item_ztje.setText(adapter.getItem(position).getBorrow_interest_rate() + "%");
			viewHolder.invest_manager_item_nhll.setText(adapter.getItem(position).getInvestor_capital() + "/" + adapter.getItem(position).getInterest_rate());
			viewHolder.invest_manager_item_dbjg.setVisibility(View.GONE);
			viewHolder.invest_manager_item_dbjg2.setVisibility(View.GONE);
			if (tipsType == 0) {
				viewHolder.invest_manager_item_tzsj.setText(adapter.getItem(position).getAdd_time());
				viewHolder.invest_manager_item_dqsj.setText(adapter.getItem(position).getDeadline());
			}

			if (tipsType == 1) {
				viewHolder.invest_manager_item_nhll.setText(adapter.getItem(position).getPeriod() + "/" + adapter.getItem(position).getTotal_period());
			...
			if (tipsType == 2) {
				viewHolder.invest_manager_item_nhll.setText(adapter.getItem(position).getPeriod() + "/" + adapter.getItem(position).getTotal_period());
				viewHolder.invest_manager_item_tzsj.setText(adapter.getItem(position).getMoney());
				viewHolder.invest_manager_item_dqsj.setText(adapter.getItem(position).getBuy_time());
			}
			...
			if (tipsType == 3) {
				...
				viewHolder.invest_manager_item_dqsj.setText(adapter.getItem(position).getMoney());
			}
			if (tipsType == 4) {
				viewHolder.invest_manager_item_ztje.setText(adapter.getItem(position).getUser_name() + "");
				viewHolder.invest_manager_item_nhll.setText(adapter.getItem(position).getHas_pay() + "/" + adapter.getItem(position).getTotal_period());
				viewHolder.invest_manager_item_tzsj.setText(adapter.getItem(position).getMoney());
				viewHolder.invest_manager_item_dqsj.setText(adapter.getItem(position).getCapital() + "");
				viewHolder.invest_manager_item_dbjg.setVisibility(View.VISIBLE);
				viewHolder.invest_manager_item_dbjg2.setVisibility(View.VISIBLE);
				viewHolder.invest_manager_item_dbjg.setText("已收利息:" + adapter.getItem(position).getInterest());
				viewHolder.invest_manager_item_dbjg2.setText("应收日期:" + adapter.getItem(position).getDeadline());
			}
			if (tipsType == 5) {
				viewHolder.invest_manager_item_ztje.setText(adapter.getItem(position).getBorrow_interest_rate() + "%");
				viewHolder.invest_manager_item_nhll.setText(adapter.getItem(position).getPeriod() + "/" + adapter.getItem(position).getTotal_period());
				viewHolder.invest_manager_item_tzsj.setText(adapter.getItem(position).getMoney());
				viewHolder.invest_manager_item_dqsj.setText(adapter.getItem(position).getCancel_times());
				viewHolder.invest_manager_item_dbjg.setVisibility(View.VISIBLE);
				viewHolder.invest_manager_item_dbjg2.setVisibility(View.VISIBLE);
				viewHolder.invest_manager_item_dbjg.setText("备注:" + adapter.getItem(position).getRemark());
				viewHolder.invest_manager_item_dbjg2.setText("撤销时间:" + adapter.getItem(position).getCancel_time());
			}
			return convertView;
		}
	...
	private static class ViewHolder {
		...
		TextView invest_manager_item_dbjg2;
	...
	private void cheangeTips() {
		...
		case 3:
			...
			break;
		case 4:
			tipsData = InvestManagerDebetActivity.this.getResources().getStringArray(R.array.debet_tips5);
			break;
		case 5:
			tipsData = InvestManagerDebetActivity.this.getResources().getStringArray(R.array.debet_tips6);
			break;
	...
	public void showDebetHtml(JsonBuilder builder) {
		...
		BaseHttpClient.post(getBaseContext(), Default.agreement, builder, new JsonHttpResponseHandler() {
						...
						if (json.getInt("status") == 1) {
							...
							if (json.has("message")) {
								...
								if (!SystenmApi.isNullOrBlank(url)) {
									...
									MyLog.e("3333333333333333333333");
	...
	public void cancleDebetAction(JsonBuilder builder, final InvestManageItem item) {
		...
		BaseHttpClient.post(getBaseContext(), Default.cancel_debt, builder, new JsonHttpResponseHandler() {
