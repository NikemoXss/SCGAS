...
public class CustomImageView extends LinearLayout implements OnTouchListener {
	...
	protected void addSubView() {
		...
		options = new DisplayImageOptions.Builder().resetViewBeforeLoading(true).cacheOnDisk(true).imageScaleType(ImageScaleType.EXACTLY).bitmapConfig(Bitmap.Config.RGB_565).considerExifParams(true).displayer(new FadeInBitmapDisplayer(300)).build();

		ImageLoader.getInstance().displayImage(URL, showImageView, options, new SimpleImageLoadingListener() {
			@Override
	...
	private void minZoom() {
		minScaleR = Math.min((float) Default.dm.widthPixels / (float) bitmap.getWidth(), (float) Default.dm.heightPixels / (float) bitmap.getHeight());
		if (minScaleR < 1.0) {
