...
public class CreateGesturePasswordActivity extends Activity implements OnClickListener {
	 ...
	 */
	enum LeftButtonMode {
		Cancel(android.R.string.cancel, true), CancelDisabled(android.R.string.cancel, false), Retry(R.string.lockpattern_retry_button_text, true), RetryDisabled(R.string.lockpattern_retry_button_text, false), Gone(ID_EMPTY_MESSAGE, false);
	...
	public void finish() {
		...
		if (modify_pass_flg) {
			...
			startActivity(new Intent(CreateGesturePasswordActivity.this, UnlockGesturePasswordActivity.class));
		}
		super.finish();
	}
	...
	private void updatePreviewViews() {
		...
		for (LockPatternView.Cell cell : mChosenPattern) {
			MyLog.i("way", "cell.getRow() = " + cell.getRow() + ", cell.getColumn() = " + cell.getColumn());
	...
	private void saveChosenPatternAndFinish() {
		...
		editor.commit();
		LocationApplication.getInstance().getLockPatternUtils().saveLockPattern(mChosenPattern);
		showToast("密码设置成功");
