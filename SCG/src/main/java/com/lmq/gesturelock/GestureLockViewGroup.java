...
import com.czscg.R;
import com.lmq.gesturelock.GestureLockView.Mode;
...
public class GestureLockViewGroup extends RelativeLayout {
	...
	private List<Integer> mAnswerList = new ArrayList<Integer>();
	...
	private int checkAnswer() {
		SharedPreferences sharedPreferences = context.getSharedPreferences("scenelist", Context.MODE_PRIVATE);
		String liststr1 = sharedPreferences.getString(Utils.SCENE_LIST, "");
		// String username=sharedPreferences.getString(Utils.SCENE_USER, "");
		@SuppressWarnings("rawtypes")
		List showSceneList = new ArrayList<Integer>();
		try {
			showSceneList = Utils.String2SceneList(liststr1);
		...
		// if(!username.equals(getNP())){//该账号没有设置手势
		// return 3;
		// }
		// 第一次绘制
		int s = showSceneList.size();
		if (isFirst == 1) {
			s = 0;
		}
		int m = mChoose.size();
		if (s == 0 && m > 0) {
			...
			try {
				String liststr = Utils.SceneList2String(mChoose);
				edit.putString(Utils.SCENE_LIST, liststr);
				edit.putString(Utils.SCENE_USER, getNP().get(0));
				edit.putString(Utils.SCENE_PWD, getNP().get(1));
				edit.commit();
			} catch (IOException e) {
				e.printStackTrace();
			}
			isFirst = 3;
			return 0;// 第一次绘制成功
		} else if (s > 0 && m > 0 && s == m) {// 再次绘制
			for (int i = 0; i < showSceneList.size(); i++) {
				if (!showSceneList.get(i).equals(mChoose.get(i))) {
					if (isFirst == 0) {
						return 3;
					}
					return 1;// 第二次匹配失败
	...
	public List<String> getNP() {
		...
		if (!sp.getString(Default.userName, "").equals("")) {
			userName = sp.getString(Default.userName, "");
			pwd = sp.getString(Default.userPassword, "");
			list.add(userName);
			list.add(pwd);
			return list;
		}
	 ...
	 * @param answer
	 */
	public void setAnswer(int isFirst) {
